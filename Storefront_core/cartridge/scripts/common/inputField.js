...
module.exports = function (pdict) {
	...
	*/
	...
	switch (element) {
		case 'select':
			input = '<select class="form-control input-select ' + fieldClass + '" id="' + id + '" name="' + name + '" ' + attributes + '>';
	...
	caption = '<div class="form-caption error' + (hasError ? ' error-message' : '') + '">' + message + '</div>';
